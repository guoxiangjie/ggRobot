...
bool aimdk_msgs__msg__path__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("aimdk_msgs.msg._path.Path", full_classname_dest, 25) == 0);
  }
  aimdk_msgs__msg__Path * ros_message = _ros_message;
  {  // path_id
  ...
  {  // points
    ...
    if (!geometry_msgs__msg__Pose2D__Sequence__init(&(ros_message->points), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create geometry_msgs__msg__Pose2D__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    geometry_msgs__msg__Pose2D * dest = ros_message->points.data;
...
PyObject * aimdk_msgs__msg__path__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("aimdk_msgs.msg._path");

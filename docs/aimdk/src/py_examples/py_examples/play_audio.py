...
class PlayAudioClient(Node):
    def __init__(self):
        super().__init__('PlayAudioClient')
        self.client = self.create_client(
            PlayAudioFile, '/aimdk_5Fmsgs/srv/PlayAudioFile')
        self.get_logger().info('✅ PlayAudio client node created.')
    ...
    def send_request(self, media_path):
        ...
        req.file.file_name = os.path.basename(media_path)
        req.file.file_path = os.path.dirname(media_path) + '/'
        req.file.priority = 6
        ...
        for i in range(5):
            ...
            if not rclpy.ok():
                self.get_logger().warn('Interrupted while waiting')
                return False
        ...
        if resp.reponse.status.value == 1:

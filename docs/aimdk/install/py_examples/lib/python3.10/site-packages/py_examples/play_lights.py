...
class PlayLightsClient(Node):
    def __init__(self):
        ...
        self.client = self.create_client(
            SetPmuLed, '/aimdk_5Fmsgs/srv/SetPmuLed')
    ...
    def send_request(self, led_mode, r, g, b, priority=6, reset_priority=False):
        ...
        self.get_logger().info(
            f'📨 Sending request to control led strip: mode={led_mode}, RGB=({r}, {g}, {b}), priority={priority}, reset_priority={reset_priority}')
        for i in range(4):
...
def main(args=None):
    ...
    try:
        ...
        if len(sys.argv) > 6:
            ...
            reset_priority = sys.argv[6].lower() in ('true', '1', 'yes')
        else:
            # interactive input
            ...
            reset_priority = input(
                "Reset priority after command? (y/n, default=n): ").strip().lower() == 'y'

        node = PlayLightsClient()
        node.send_request(led_mode, r, g, b, priority, reset_priority)

    ...
    while True:
        ...
        try:
            await publish("sensor.all", {
                ...
                "emergency": _node.emergency if _node.emergency else None,
            })
...
async def cloud_pusher(interval: float = 1.0):
    ...
    GetRealtimeMapData 直接给 cur_pos + lidar_points + trajectory + base64 png 底图，
    这里透传 JSON（建图工作台 A3 数据源）。0.5s 一拉（建图实时数据接口，
    文档限频约束针对状态类轮询，此为官方实时数据通道）。
    """
    while True:
        await asyncio.sleep(interval)
        from .. import node as node_mod
        ...
        try:
            from .. import rpc
            r = await asyncio.to_thread(rpc.mapping_realtime)
            data = r.get("data", {})
            if data:
                await publish("slam.map", data)
        except Exception:
            pass   # 单次拉取失败跳过，下轮重试
